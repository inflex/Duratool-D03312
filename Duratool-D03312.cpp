...
void show_help(void) {
	wprintf(L"Duratool D03312 Multimeter serial data decoder\r\n"
			...
			"\r\n"
			"\texample: Duratool-D03312.exe -z 120 -p 4 -s 9600:8n1 -m -fc #10ff10 -bc #000000 -wx 480 -wy 60 -fw 600\r\n"
			, BUILD_VER
			, BUILD_DATE 
			);
} 
...
void enable_coms(struct glb *pg, wchar_t *com_port) {
	...
	if (com_read_status == FALSE) {
		wprintf(L"Error setting com port configuration (9600:8n1 etc)\r\n");
		CloseHandle(hComm);
		exit(1);
	} else {
...
bool auto_detect_port(struct glb *pg) {
	...
	uint8_t d[50];      // Serial data packet, only need 22 bytes really, but we'll go to 50 for the buffer 
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR lpCmdLine, int nCmdShow) {
	...
	wchar_t logmode[SSIZE]; // Mode suffix name we put in the mmdata file
	...
	wc.lpszClassName = L"Duratool D03312 Meter";
	...
	hstatic = CreateWindowW(wc.lpszClassName, L"Duratool D03312 Meter", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 50, 50, g.window_x, g.window_y, NULL, NULL, hInstance, NULL);
	...
	if (0) {
		...
		while (msg.message != WM_QUIT) {
			...
			if (0) {
				...
				if (com_read_status == FALSE) {
					...
					i = 0;
					int fail_countdown = DATA_FRAME_SIZE *2;
					do {
						unsigned char temp_char;
						com_read_status = ReadFile(hComm, &temp_char, sizeof(temp_char), &bytes_read, NULL);
						//			bytes_read = read(g.serial_params.fd, &temp_char, 1);
						if (bytes_read) {
							if (i == 0 && temp_char != 0xAA) {
								fail_countdown--;
								if (g.debug) fprintf(stderr,"%02x not start of frame, waiting...\n", temp_char);
								if (fail_countdown < 1) {
									break;
								}

								continue; // first char is AA
							}
					...
					if (d[18] & 0x80) StringCbPrintf(mmmode,sizeof(mmmode),L"REL");
					if (d[18] & 0x20) StringCbPrintf(mmmode,sizeof(mmmode),L"AUTO");

					if (d[19] & 0x40) StringCbPrintf(mmmode,sizeof(mmmode),L"hFE");
					if (d[19] & 0x20) StringCbPrintf(mmmode,sizeof(mmmode),L"%");
					if (d[19] & 0x08) StringCbPrintf(mmmode,sizeof(mmmode),L"MIN");
					if (d[19] & 0x20) StringCbPrintf(mmmode,sizeof(mmmode),L"min-max");
					if (d[19] & 0x20) StringCbPrintf(mmmode,sizeof(mmmode),L"MAX");
					if (d[19] & 0x20) StringCbPrintf(mmmode,sizeof(mmmode),L"USB");

					if (d[20] & 0x80) StringCbPrintf(units,sizeof(units),L"F");
					if (d[20] & 0x40) StringCbPrintf(prefix,sizeof(prefix),L"n");
					if (d[20] & 0x20) StringCbPrintf(prefix,sizeof(prefix),L"%s",uu);
					if (d[20] & 0x02) StringCbPrintf(units,sizeof(units),L"%sF",dd);
					if (d[20] & 0x01) StringCbPrintf(units,sizeof(units),L"%sC",dd);

					if (d[21] & 0x80) StringCbPrintf(units,sizeof(units),L"Hz");
					if (d[21] & 0x40) StringCbPrintf(units,sizeof(units),L"%s",oo);
					if (d[21] & 0x20) StringCbPrintf(prefix,sizeof(prefix),L"k");
					if (d[21] & 0x10) StringCbPrintf(prefix,sizeof(prefix),L"M");
					if (d[21] & 0x08) StringCbPrintf(units,sizeof(units),L"V");
					if (d[21] & 0x04) StringCbPrintf(units,sizeof(units),L"A");
					if (d[21] & 0x02) StringCbPrintf(prefix,sizeof(prefix),L"m");
					if (d[21] & 0x01) StringCbPrintf(prefix,sizeof(prefix),L"%s",uu);
				...
				char lmode[128];
				{
					if (d[21] & (1<<3)) { 
						// volts showing
						if (d[10] & (1<<0)) snprintf(lmode, sizeof(lmode),"DIODE");
						else {
							if (d[10] & (1<<2)) snprintf(lmode, sizeof(lmode),"VOLTSDC");
							else snprintf(lmode, sizeof(lmode),"VOLTSAC");
						}
					}
					if (d[21] & (1<<6)) snprintf(lmode, sizeof(lmode),"OHMS");
					if (d[21] & (1<<7)) snprintf(lmode, sizeof(lmode),"FREQ");
					if (d[20] & (0x03)) snprintf(lmode, sizeof(lmode),"TEMP");
					if (d[21] & (1<<2)) {
						if (d[10] & (1<<2)) snprintf(lmode, sizeof(lmode),"AMPSDC");
						else snprintf(lmode, sizeof(lmode),"AMPSAC");
					}
				...
				if (g.mmdata_active == 1) {
					...
					if (f) {
						if (g.debug) { wprintf(L"file exists, not overwriting\r\n", i); }
						fclose(f);
					} 
					else {
						char output[1024];
						...
						if (f) {
							if (g.debug) { wprintf(L"Writing new mmdata file..."); }

							wcstombs(output, linetmp, sizeof(output)-1);
							fprintf(f,"%s\t%s",output, lmode);


							fflush(f);
							fclose(f);
							if (g.debug) { wprintf(L"Renaming mmdata file..."); }
							_wrename(g.mmdata_output_temp_file, g.mmdata_output_file);
							if (g.debug) { wprintf(L"mmdata written.\r\n"); }
						}
						else {
							wprintf(L"Problem attempting to open '%s' for writing\r\n", g.mmdata_output_temp_file);
						}
					}
				}
		...
		LRESULT CALLBACK WindowProcedure(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
			switch (message) /* handle the messages */
			{
				...
				case WM_CREATE: 
					RegisterHotKey(hwnd, ID_QUIT, MOD_ALT, 0x51);
					break;
