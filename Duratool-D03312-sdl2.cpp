...
int init(struct glb *g) {
	...
	g->serial_config = default_serial_config;
...
int main ( int argc, char **argv ) {
	...
	while (!quit) {
		...
		i = 0;
		fail_countdown = DATA_FRAME_SIZE *2;
		do {
		...
		if (d[18] & 0x80) snprintf(mmmode,sizeof(mmmode),"REL");
		if (d[18] & 0x20) snprintf(mmmode,sizeof(mmmode),"AUTO");

		if (d[19] & 0x40) snprintf(mmmode,sizeof(mmmode),"hFE");
		if (d[19] & 0x20) snprintf(mmmode,sizeof(mmmode),"%%");
		if (d[19] & 0x08) snprintf(mmmode,sizeof(mmmode),"MIN");
		if (d[19] & 0x20) snprintf(mmmode,sizeof(mmmode),"min-max");
		if (d[19] & 0x20) snprintf(mmmode,sizeof(mmmode),"MAX");
		if (d[19] & 0x20) snprintf(mmmode,sizeof(mmmode),"USB");

		if (d[20] & 0x80) snprintf(units,sizeof(units),"F");
		if (d[20] & 0x40) snprintf(prefix,sizeof(prefix),"n");
		if (d[20] & 0x20) snprintf(prefix,sizeof(prefix),"%s",uu);
		if (d[20] & 0x02) snprintf(units,sizeof(units),"%sF",dd);
		if (d[20] & 0x01) snprintf(units,sizeof(units),"%sC",dd);

		if (d[21] & 0x80) snprintf(units,sizeof(units),"Hz");
		if (d[21] & 0x40) snprintf(units,sizeof(units),"%s",oo);
		if (d[21] & 0x20) snprintf(prefix,sizeof(prefix),"k");
		if (d[21] & 0x10) snprintf(prefix,sizeof(prefix),"M");
		if (d[21] & 0x08) snprintf(units,sizeof(units),"V");
		if (d[21] & 0x04) snprintf(units,sizeof(units),"A");
		if (d[21] & 0x02) snprintf(prefix,sizeof(prefix),"m");
		if (d[21] & 0x01) snprintf(prefix,sizeof(prefix),"%s",uu);
		...
		if (fail_countdown < 1) {
			snprintf(line1, sizeof(line1), "Check com");
		} 
		...
		if (!g.quiet) fprintf(stderr,"%s\t%s\r",logline, lmode); fflush(stderr);
